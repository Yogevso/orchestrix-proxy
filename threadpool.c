...
threadpool *create_threadpool(int num_threads_in_pool, int max_queue_size) {
    ...
    for (int i = 0; i < num_threads_in_pool; i++) {
        if (pthread_create(&(pool->threads[i]), NULL, do_work, (void *)pool) != 0) {
            ...
            /* Shut down the threads that were already created. */
            pool->num_threads = i;
...
int dispatch(threadpool *from_me, dispatch_fn dispatch_to_here, void *arg) {
    ...
    /* Non-blocking: reject immediately when the queue is full. */
...
void destroy_threadpool(threadpool *destroyme) {
    ...
    /* Wake any dispatchers blocked on a full queue so they can exit. */
    pthread_cond_broadcast(&(destroyme->q_not_full));

    /* Wait for all queued work to drain. */
